...
mod tests {
    ...
    async fn test_delete_environment() {
        let (_tmp, pool) = setup().await;
        let env_id = seed_env(&pool, "prod").await;
        delete_environment(&pool, &env_id).await.unwrap();
        let gone = get_environment(&pool, &env_id).await.unwrap();
        assert!(gone.is_none());
    }
    ...
    async fn test_migrate_legacy_creates_default_credential() {
        let (_tmp, pool) = setup().await;
        // 直接走 SQL 模拟一个未迁移的旧环境（绕过 save_environment）
        sqlx::query(
